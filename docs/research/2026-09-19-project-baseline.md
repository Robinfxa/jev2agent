...
目录包含展开的 Codex 架构交接包、同名原始 ZIP、会话迁移 ZIP 和会话背景便笺。交接标识为 `codex-handoff-2026-09-18`，设计收束状态为 SEALED_FOR_IMPLEMENTATION，不等于 HOST_VERIFIED 或 DEPLOYMENT_APPROVED。
...
- 两个根 ZIP 的 CRC 检查通过；保留原文件。根 ZIP 是重复迁移载体，Git 忽略它们；封包内部两个历史 ZIP 保持跟踪以维持清单完整。
...
- GitHub API 与 `git ls-remote` 确认目标仓库为空。初始化本地 main、配置 origin 并保存初始提交；本轮不推送。
