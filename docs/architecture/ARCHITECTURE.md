...
宿主由用户指定为 Codex；本包工程落点为 **Codex CLI、单任务非交互运行、本地 stdio MCP 搜索/召回工具**。MCP 是本包选择的适配路径，不是本轮已核验的 Codex 版本能力。原封包环境未发现 Codex 命令、未进行网页核验；这是历史环境记录，当前宿主事实仍须在 W1a 核验。

本稿以结项基线合并而来，采用 PATCH-01（批准与验证分开）、PATCH-02（仅 ADMIT＋recall 首发）、PATCH-03（弃权与失败分流），本次仅调整文档布局与引用，合同语义不变。细节依据保存在 [历史章节索引](../references/README.md)。本轮新增的 Codex/事务存储/单任务授权域等属于**工程裁定**，见 [变更及决议处置](../decisions/DECISION_CLOSURE.md)，不伪装成原材料已经指定。
...
依据：[MASTER，L71–161](../references/MASTER.txt)；[M17，L5–38](../references/M17.txt)；[CUR，L5–9](../references/CUR.txt)。
...
依据：[MASTER，L177–236](../references/MASTER.txt)；[M03，L9–39](../references/M03.txt)；[M15，L9–21](../references/M15.txt)。
...
依据：[A5，L1–33](../references/A5.txt)；[M04，L9–36](../references/M04.txt)；[M14，L11–20](../references/M14.txt)。
...
依据：[M04，L18–64](../references/M04.txt)；[M04，L66–98](../references/M04.txt)；[M08，L23–37](../references/M08.txt)。
...
依据：[M05，L15–35](../references/M05.txt)；[M05，L55–61](../references/M05.txt)；[M06，L84–105](../references/M06.txt)；[M08，L79–105](../references/M08.txt)。旧 §05 摘要按 PATCH-03 澄清，主规则仍归 §06.11。
...
依据：[M03，L35–39](../references/M03.txt)；[M03，L55–71](../references/M03.txt)；[M06，L49–78](../references/M06.txt)。
...
依据：[M09，L9–33](../references/M09.txt)；[M09，L65–75](../references/M09.txt)；[M09，L85–113](../references/M09.txt)；[M15，L23–55](../references/M15.txt)。
...
依据：[M14，L58–90](../references/M14.txt)；[M11，L11–61](../references/M11.txt)；[M11，L65–79](../references/M11.txt)。
...
依据：[M12，L21–77](../references/M12.txt)；[M12，L117–153](../references/M12.txt)；[M13，L9–15](../references/M13.txt)；[M13，L71–87](../references/M13.txt)；[M16，L15–26](../references/M16.txt)；[E4，L11–36](../references/E4.txt)。

## B9｜本次收束、适配优先级与状态

逻辑基线及三处文档修订纳入本次实施交接；Codex 首发形态和适配裁定见 [Codex 合同](CODEX_HOST_CONTRACT.md)。这不是对旧历史材料的全部事实背书。
...
F1–F3 已从“未选方案”转成具体 Codex 实施/验收任务，**运行事实尚未关闭**。F1 前置边界与交付、F2 可信任务/权限、F3 来源恢复都须按 [E0](../validation/E0_CODEX_ACCEPTANCE.md) 验证。不拿状态改名消除未核验条件。
