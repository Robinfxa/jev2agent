# Semantic Reflex Runtime｜首发逻辑架构基线（冻结候选）

**基于 v0.6 的合并阅读稿，不是 v0.7。状态：作者侧 Reviewed；待项目负责人批准。** 本文只整理现行首发合同，并显式采用 [三处修订候选](PATCHES.md)。没有新增产品能力，不选择宿主/后端/语言，不将原 Proposed 改为 Accepted。

**适用范围**：宿主无关的逻辑与验证合同。具体宿主的可实施性、权限/持久化/实际交付映射仍开放。本文不是完整生产 API、部署说明或收益证明。章节语义仍归原 §03–§17 和 A5；原文带行号副本见 [证据索引](evidence/INDEX.md)。历史实验与推演不是新增运行时规则。
...
先使用 Harness-native、Static instructions、Deterministic policies，再考虑 Semantic Reflex。C 默认不启用；只在明确授权的实验或已验证范围内使用。主 agent loop、工具权限、沙箱、最终验收仍属于既有宿主。
...
依据：[MASTER，L71–161](evidence/MASTER.txt)；[M17，L5–38](evidence/M17.txt)；[CUR，L5–9](evidence/CUR.txt)。

## B1｜一个流程，六项逻辑责任，不是六个服务

```text
宿主已授权、完成的只读工具返回（接收方首次读取之前）
...
六项责任为 Host Adapter、Source/State、Predicate Backend、Policy/Executor、Recall、Telemetry/Evaluation。同进程模块或原生组件都可承载；物理形态要由宿主事实决定，不按架构图数量创建服务。

依据：[MASTER，L177–236](evidence/MASTER.txt)；[M03，L9–39](evidence/M03.txt)；[M15，L9–21](evidence/M15.txt)。
...
字段族、生产/消费关系按 A5 主定义索引，不把本地 lab JSON 自动升级为生产 wire。JSON、Python、目录名和测试身份均不是技术栈或认证选型。

依据：[A5，L1–33](evidence/A5.txt)；[M04，L9–36](evidence/M04.txt)；[M14，L11–20](evidence/M14.txt)。
...
只有源与最低决策/恢复记录就绪后才承诺可延后；有效 run/handle 内不因普通容量淘汰删源。容量耗尽停新延后，原文/最低记录与普通 telemetry 轮转分离。具体保留上限、任务结束点与权限删除要求由真实部署合同确定。

依据：[M04，L18–64](evidence/M04.txt)；[M04，L66–98](evidence/M04.txt)；[M08，L23–37](evidence/M08.txt)。
...
依据：[M05，L15–35](evidence/M05.txt)；[M05，L55–61](evidence/M05.txt)；[M06，L84–105](evidence/M06.txt)；[M08，L79–105](evidence/M08.txt)。旧 §05 摘要按 PATCH-03 澄清，主规则仍归 §06.11。
...
依据：[M03，L35–39](evidence/M03.txt)；[M03，L55–71](evidence/M03.txt)；[M06，L49–78](evidence/M06.txt)。
...
Compaction/重启是否在支持范围内须有证据。范围可以明确收窄，但不能让真实运行不可避免跨界后仍承诺引用可用。具体重复 ID/逐项失败/wire enum 由宿主绑定时冻结，语义要求是不得静默丢项并伪称完整成功。

依据：[M09，L9–33](evidence/M09.txt)；[M09，L65–75](evidence/M09.txt)；[M09，L85–113](evidence/M09.txt)；[M15，L23–55](evidence/M15.txt)。
...
依据：[M14，L58–90](evidence/M14.txt)；[M11，L11–61](evidence/M11.txt)；[M11，L65–79](evidence/M11.txt)。
...
依据：[M12，L21–77](evidence/M12.txt)；[M12，L117–153](evidence/M12.txt)；[M13，L9–15](evidence/M13.txt)；[M13，L71–87](evidence/M13.txt)；[M16，L15–26](evidence/M16.txt)；[E4，L11–36](evidence/E4.txt)。

## B9｜变更、停止和批准边界

允许四种结果：C 在已验证范围有效；B 有效而删除 C；仅留少数窄场景；停止整个切片。不能为补 C/A 的坏账自动加 OBSERVE，也不因已写代码必须保留它。

本文获批准仅冻结宿主无关逻辑；F1–F3 仍阻塞具体宿主适配完成；上线另经实验与授权。当前没有自动把任何 DEC 改 Accepted。后续只有明确新需求、真实宿主冲突、可复验合同反例或跨章不兼容才重开受影响条款。

实施者的首个入口是 [E0 原合同](evidence/E0.txt) 和 [待办分流](DECISIONS-AND-OPEN-ITEMS.md)，不是重新阅读六轮历史。细节存在不同解释时，先查唯一主定义；仍不一致则登记具体冲突，不擅自选对自己最方便的旧段。
